...
Duk.Widget = function(parameters, style, manager, root, parent) {
   ...
   this.addWidget = function(parameters) {
	   deep = p.deepCopy(p.merge(parameters, {'widget': true}));
	   deep['parent'] = me.calc;
     ...
     * @param   {Number}    [x]     The x position we want to move it to (relative)
     * @param   {Number}    [y]     The y position we want to move it to (relative)
     * @param   {Duk.Enums.Orientation} [orientation] The orientation of the move
     */
    this.move = function(x, y, orientation){
   ...
   this.populate = function() {
	...
	this.rebuildMap = function(){
	   ...
	   for(var i = 0; i < me.widgets.length; i++){
		   ...
		   width = parseInt(me.widgets[i].dimensions.width);
